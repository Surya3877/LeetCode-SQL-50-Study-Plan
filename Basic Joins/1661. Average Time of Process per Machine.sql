...
(machine_id, process_id, activity_type) is the primary key (combination
...
There is a factory website that has several machines each running the samr=e number of
processes. Write a solution to find the average time each machine takes to complete a process.

The time to complete a process is the 'end' timestamp minus the 'start' timestamp. The
average time is calculated by the total time to complete every process on the machine divided by
the number of the processes that were run.
...
+------------+-----------------+
Explanation:
There are 3 machines running 2 process each.
...
CREATE DATABASE IF NOT EXISTS 1661_Average_Time_of_Process_Per_Machine;
USE 1661_Average_Time_of_Process_Per_Machine;
...
INSERT INTO Activity (machine_id, process_id, activity_type, timestamp) VALUES ('2', '1', 'end', '2.500');
